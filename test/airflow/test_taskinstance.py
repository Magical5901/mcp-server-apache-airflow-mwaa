...
def _patch_api(method_name, return_value):
    """Patch the per-call TaskInstanceApi factory so ``method_name`` returns ``return_value``.

    Returns (context_manager, mock_method) — the mock_method is what tests assert against.
    """
    mock_api = MagicMock()
    getattr(mock_api, method_name).return_value = return_value
    cm = patch("src.airflow.taskinstance._task_instance_api", return_value=mock_api)
    return cm, getattr(mock_api, method_name)
...
class TestTaskInstanceModule:
    ...
    async def test_get_task_instance(self, dag_id, task_id, dag_run_id, expected_state):
        ...
        cm, mock_get = _patch_api("get_task_instance", mock_response)
        with cm:
            result = await get_task_instance(
                ENV, PROFILE, REGION, dag_id=dag_id, task_id=task_id, dag_run_id=dag_run_id
            )
    ...
    async def test_list_task_instances(self, params):
        ...
        cm, mock_get = _patch_api("get_task_instances", mock_response)
        with cm:
            result = await list_task_instances(ENV, PROFILE, REGION, **params)
    ...
    async def test_update_task_instance(self, dag_id, dag_run_id, task_id, state):
        ...
        cm, mock_patch = _patch_api("patch_task_instance", mock_response)
        with cm:
            result = await update_task_instance(
                ENV, PROFILE, REGION, dag_id=dag_id, dag_run_id=dag_run_id, task_id=task_id, state=state
            )
    ...
    async def test_list_task_instance_tries(self, dag_id, dag_run_id, task_id, limit, offset, order_by):
        ...
        cm, mock_get = _patch_api("get_task_instance_tries", mock_response)
        with cm:
